...
public class ObstacleList {
	...
	*/
	public boolean badPath(RRTNode destinationNode, RRTNode currentNode){
		ItemPosition destination = new ItemPosition("NodeToIPDes", destinationNode.position.getX(), destinationNode.position.getY(), 0);
		ItemPosition current = new ItemPosition("NodeToIPCurrt", currentNode.position.getX(), currentNode.position.getY(), 0);
		boolean check = false;
		for(int i=0; i< ObList.size() && ObList.elementAt(i) != null; i++){
			if(ObList.elementAt(i).checkCross(destination, current)){
				return true;
	...
	public void detected(ItemPosition blocker){
		...
		if(uncontained){
			Obstacles newObs = new Obstacles(blocker.getX(), blocker.getY());
